...
onReady(async function () {
    ...
    function nestedRepliesFeature() {
        ...
        function parsePostLink(href) {
            const m = href.match(/^\/([^\/]+)\/res\/(\d+)\.html#(\d+)$/);
            if (m) return { board: m[1], thread: m[2], post: m[3], quoteUrl: href };
            const n = href.match(/^#q(\d+)$/);
            if (n) return { board: null, thread: null, post: n[1], quoteUrl: href };
            return null;
        }

        function isPostNumberLink(href) {
            return /^#q\d+$/.test(href);
        }

        function getOrCreateReplyPreview(innerPost) {
        ...
        function findInlineQuote(quoteUrl, innerPost) {
            return innerPost.querySelector(`.inlineQuote[data-quote-url="${quoteUrl}"]`);
        }

        function loadInlinePost(quoteTarget, sourceLink, replyPreview) {
            let postNode = document.getElementById(quoteTarget.post);
            if (!postNode) {
                const placeholder = document.createElement('div');
                placeholder.className = 'inlineQuote';
                placeholder.dataset.quoteUrl = quoteTarget.quoteUrl;
                placeholder.textContent = 'Post not found.';
                replyPreview.appendChild(placeholder);
                return;
            }
            const inlineQuote = postNode.cloneNode(true);
            inlineQuote.classList.add('inlineQuote');
            inlineQuote.dataset.quoteUrl = quoteTarget.quoteUrl;

            // Recursively process quotes in the new inline post
            Array.from(inlineQuote.querySelectorAll('.linkQuote, .quoteLink, .panelBacklinks > a')).forEach(a => {
                a.addEventListener('click', onQuoteClick, true);
            });

            replyPreview.appendChild(inlineQuote);
        }

        function onQuoteClick(e) {
            const quote = e.currentTarget;
            const href = quote.getAttribute('href');
            if (!href || isPostNumberLink(href)) return;

            const innerPost = closestAncestor(quote, '.innerPost, .innerOP');
            if (!innerPost) return;

            e.preventDefault();
            e.stopPropagation();

            const quoteTarget = parsePostLink(href);
            if (!quoteTarget) return;

            const replyPreview = getOrCreateReplyPreview(innerPost);

            // If already nested, remove it and remove dashed underline
            const existing = findInlineQuote(quoteTarget.quoteUrl, innerPost);
            if (existing) {
                existing.remove();
                quote.classList.remove('nestedQuoteLink');
                return;
            }

            // Otherwise, nest and add dashed underline
            loadInlinePost(quoteTarget, quote, replyPreview);
            quote.classList.add('nestedQuoteLink');
        }

        function setupDelegatedQuoteClicks() {
            document.body.addEventListener('click', function (e) {
                const target = e.target;
                if (
                    target.matches('.linkQuote, .quoteLink, .panelBacklinks > a') &&
                    !isPostNumberLink(target.getAttribute('href'))
                ) {
                    onQuoteClick(e);
                }
            }, true);
                ...
                mutations.forEach(mutation => {
                    mutation.addedNodes.forEach(node => {
                        ...
                        node.querySelectorAll &&
                            node.querySelectorAll('.linkQuote, .quoteLink, .panelBacklinks > a')
                                .forEach(a => {
                                    if (!isPostNumberLink(a.getAttribute('href'))) {
                                        a.addEventListener('click', onQuoteClick, true);
                                    }
                                });
                    });
        ...
        function initNestedReplies() {
            setupDelegatedQuoteClicks();
            setupMutationObserver();
        }

        if (document.readyState === "loading") {
            document.addEventListener("DOMContentLoaded", initNestedReplies);
        } else {
            initNestedReplies();
        }
    }
