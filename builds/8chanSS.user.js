...
onReady(async function () {
    ...
    try {
        ...
        if (isDesktop && themeSelector && themeSelector.parentNode) {
            ...
            const parent = themeSelector.parentNode;
            parent.insertBefore(
                openBracketSpan,
                themeSelector.nextSibling
            );
            parent.insertBefore(link, openBracketSpan.nextSibling);
            
            closeBracketSpan = document.createElement("span");
            closeBracketSpan.textContent = "\u00A0]";
            parent.insertBefore(closeBracketSpan, link.nextSibling);
        }
    } catch (err) {
        console.warn("8chanSS: Failed to inject menu icon into page header/sidebar:", err);
    }
